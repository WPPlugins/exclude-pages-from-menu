...
class Exclude_Pages_From_Menu_Admin {
	 ...
	 *
	 */
	function Exclude_Pages_From_Menu_Admin() {
		$this->__construct();
	}
	...
	function exclude_pages_from_menu_settings_link( $links, $file ) {
		...
		if ( false !== strpos( $file, 'exclude-pages-from-menu' ) ) {
			$mylinks = array(
				'<a href="http://freewptp.com/forum/wordpress-plugins-forum/exclude-pages-from-menu/">' . esc_html__( 'Get Support', 'exclude-pages-from-menu' ) . '</a>'
			);
	...
	function exclude_pages_from_menu_setup_notice(){
		...
		if ( $hascaps ) {
			...
			echo '<div class="notice notice-info is-dismissible exclude-pages-from-menu"><p>' . sprintf( __( 'To use <em>Exclude Pages From Menu plugin</em> please visit page edit screen and to get plugin support contact us on <a href="%1$s" target="_blank">plugin support forum</a> or <a href="%2$s" target="_blank">contact us page</a>.', 'exclude-pages-from-menu'), 'http://freewptp.com/forum/wordpress-plugins-forum/exclude-pages-from-menu/', 'http://freewptp.com/contact/' ) . '</p></div>';
		}
	}
	...
	public function epfm_save( $post_id ) {
		...
		update_post_meta( $post_id, '_epfm_meta_box_field', $_POST['epfm_meta_box_field'] );
	}
	...
	public function epfm_render_meta_box_content( $post ) {
		...
		<label for="epfm_meta_box_field">
		    <?php esc_html_e( 'Exclude this page.', 'exclude-pages-from-menu' ); ?>
		</label>
		<br /><br />
		<a href="#" id="epfm-help"><?php esc_html_e( 'Help', 'exclude-pages-from-menu' ); ?></a><a href="http://freewptp.com/forum/wordpress-plugins-forum/exclude-pages-from-menu/" class="alignright" target="_blank"><?php esc_html_e( 'Get Support', 'exclude-pages-from-menu' ); ?></a>
		<br /><br />
		<div id="epfm-help-wrapper" style="display: none;">
			<?php esc_html_e( 'This will only remove page from menu which is created using wp_page_menu function.', 'exclude-pages-from-menu' ); ?>
			<br /><br />
